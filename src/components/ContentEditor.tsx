...
function Field({ label, path, area = false, draft, onChange }: { label: string; path: string; area?: boolean; draft: AnniversaryContent; onChange: (path: string, value: string) => void }) {
...
function MusicField({ draft, onAudio, onChange }: { draft: AnniversaryContent; onAudio: (file: File) => void; onChange: (path: string, value: string) => void }) {
  return <div className="space-y-2"><label className="flex cursor-pointer items-center justify-between rounded-lg border border-dashed border-bloom-rose/40 bg-bloom-rose/5 p-3 text-xs text-bloom-white/70"><span>{draft.musicUrl.startsWith("data:") ? "เลือกเพลงใหม่แล้ว" : "เลือกไฟล์เพลง"}<small className="mt-1 block text-bloom-white/40">รองรับ MP3, WAV, M4A</small></span><Upload className="h-4 w-4 text-bloom-pink" /><input type="file" accept="audio/*" className="hidden" onChange={(e) => { const file = e.target.files?.[0]; if (file) onAudio(file); }} /></label>{draft.musicUrl && <button type="button" onClick={() => onChange("musicUrl", "")} className="flex w-full items-center justify-center gap-2 rounded-lg border border-red-300/30 px-3 py-2 text-xs text-red-200 transition-colors hover:bg-red-300/10"><Trash2 className="h-3.5 w-3.5" /> ลบเพลงนี้</button>}</div>;
}
